...
static crossos_key_mod_t x_state_to_mods(unsigned int state)
{
    ...
    if (state & ShiftMask)   m |= CROSSOS_MOD_SHIFT;
    if (state & ControlMask) m |= CROSSOS_MOD_CTRL;
    if (state & Mod1Mask)    m |= CROSSOS_MOD_ALT;
    if (state & Mod4Mask)    m |= CROSSOS_MOD_SUPER;
    return m;
}
...
static int keysym_to_crossos(KeySym ks)
{
    ...
    switch (ks) {
    case XK_Escape:    return CROSSOS_KEY_ESCAPE;
    case XK_Return:    return CROSSOS_KEY_ENTER;
    case XK_Tab:       return CROSSOS_KEY_TAB;
    case XK_BackSpace: return CROSSOS_KEY_BACKSPACE;
    case XK_Insert:    return CROSSOS_KEY_INSERT;
    case XK_Delete:    return CROSSOS_KEY_DELETE;
    case XK_Right:     return CROSSOS_KEY_RIGHT;
    case XK_Left:      return CROSSOS_KEY_LEFT;
    case XK_Down:      return CROSSOS_KEY_DOWN;
    case XK_Up:        return CROSSOS_KEY_UP;
    case XK_Page_Up:   return CROSSOS_KEY_PAGE_UP;
    case XK_Page_Down: return CROSSOS_KEY_PAGE_DOWN;
    case XK_Home:      return CROSSOS_KEY_HOME;
    case XK_End:       return CROSSOS_KEY_END;
    ...
    case XK_Shift_L:   return CROSSOS_KEY_LEFT_SHIFT;
    case XK_Shift_R:   return CROSSOS_KEY_RIGHT_SHIFT;
    case XK_Control_L: return CROSSOS_KEY_LEFT_CTRL;
    case XK_Control_R: return CROSSOS_KEY_RIGHT_CTRL;
    case XK_Alt_L:     return CROSSOS_KEY_LEFT_ALT;
    case XK_Alt_R:     return CROSSOS_KEY_RIGHT_ALT;
    case XK_Super_L:   return CROSSOS_KEY_LEFT_SUPER;
    case XK_Super_R:   return CROSSOS_KEY_RIGHT_SUPER;
...
static void process_xevent(XEvent *xev)
{
    ...
    case GenericEvent:
        if (s_xi2_opcode >= 0 && xev->xcookie.extension == s_xi2_opcode) {
            ...
            case XI_TouchBegin:  tev.type = CROSSOS_EVENT_TOUCH_BEGIN;  break;
            case XI_TouchUpdate: tev.type = CROSSOS_EVENT_TOUCH_UPDATE; break;
            case XI_TouchEnd:    tev.type = CROSSOS_EVENT_TOUCH_END;    break;
            default: XFreeEventData(s_display, &xev->xcookie); return;
            }
...
int crossos_touch_is_supported(void)
{
    ...
    while ((de = readdir(d)) != NULL) {
        ...
        close(fd);
        if (evbit & (1UL << EV_ABS)) { closedir(d); return 1; }
    }
    closedir(d);
    return 0;
}
