...
crossos_result_t crossos__platform_init(void)
{
    ...
    if (XQueryExtension(s_display, "XInputExtension",
        ...
        if (major < 2) s_xi2_opcode = -1; /* XI2 too old */
    }
                                        ...
                                        uint32_t flags)
{
    ...
    if (!win) { crossos__set_error("X11: OOM"); return NULL; }
    win->width  = width;
    win->height = height;
    ...
    if (flags & CROSSOS_WINDOW_FULLSCREEN)
        crossos_window_set_fullscreen(win, 1);
...
void crossos_window_destroy(crossos_window_t *win)
{
    ...
    if (win->gc)   XFreeGC(s_display, win->gc);
    if (win->xwin) XDestroyWindow(s_display, win->xwin);
    free(win->fb_pixels);
    free(win);
}
...
{
    if (win && s_display) { XMapWindow(s_display, win->xwin); XFlush(s_display); }
}

void crossos_window_hide(crossos_window_t *win)
{
    if (win && s_display) { XUnmapWindow(s_display, win->xwin); XFlush(s_display); }
}
...
{
    if (!s_display || idx != 0) return CROSSOS_ERR_PARAM;
    if (w) *w = DisplayWidth(s_display, s_screen);
    if (h) *h = DisplayHeight(s_display, s_screen);
    return CROSSOS_OK;
}
...
crossos_result_t crossos_surface_present(crossos_surface_t *surf)
{
    ...
    crossos_window_t *win = surf->win;
    if (!win->ximage) return CROSSOS_ERR_DISPLAY;
